...
export type NodeShape = 'box' | 'diamond' | 'rounded' | 'cylinder'
...
function splitToAndLabel(raw: string): { toRaw: string; label?: string } {
  ...
  if (idx === -1) {
    // Also check for trailing colon with content after
...
function stripComment(line: string): string {
  // Remove inline comments — # not inside shape delimiters
  // Simple approach: find last ` #` or `\t#` that's not inside delimiters
  let depth = 0
...
function parseLine(raw: string, lineNum: number): Statement[] {
  ...
  // Directive: @flow LR
  if (trimmed.startsWith('@')) return [] // handled separately in parse()

  // Divider: --- text --- or just ---
  if (trimmed.startsWith('---')) {
    const inner = trimmed.replace(/^-+\s*/, '').replace(/\s*-+$/, '').trim()
    return [{ kind: 'divider', label: inner || undefined } as DividerStmt]
  }

  // Block else: } else { or } else label {
  ...
  // Block start: alt/opt/loop/par label {
  ...
  // Edge or chain: split on edge tokens
  const parts = stripped.trim().split(EDGE_RE)

  if (parts.length >= 3) {
    // Chain: [node, token, node, token, node, ...]
